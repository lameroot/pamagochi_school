...
(() => {
 ...
 function refresh(){
  ...
  $('#lesson-grid').innerHTML=lessons.map((l,i)=>`<a class="lesson-card ${l.color}" href="#lesson/${l.id}"><div class="card-top"><span class="lesson-icon">${l.icon}</span><span class="card-number">${l.source||'УРОК '+(i+1)}</span>${done(l)===l.tasks.length?'<span class="completed-mark" aria-label="Пройден">✓</span>':''}</div><h3>${l.title}</h3><p>${l.subtitle}</p><div class="card-bottom"><span class="mini-stars" aria-label="Решено ${done(l)} из ${l.tasks.length}">${l.tasks.map((_,n)=>`<i class="${n<done(l)?'earned':''}">✦</i>`).join('')}</span><span>${done(l)} / ${l.tasks.length} <b>→</b></span></div></a>`).join('');
